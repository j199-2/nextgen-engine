...
async def index():
    ...
    <head>
        ...
        <style>
            .neon-border { box-shadow: 0 0 15px rgba(168, 85, 247, 0.2), inset 0 0 15px rgba(59, 130, 246, 0.1); }
            ...
            <div class="bg-slate-900/20 border border-purple-900/20 rounded-3xl p-8 backdrop-blur-md shadow-2xl neon-border space-y-6 relative overflow-hidden">
                ...
                <div id="step-input" class="space-y-6">
                    <div class="space-y-2 text-center">
                        <span class="text-[9px] uppercase font-black tracking-widest bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-2.5 py-1 rounded-full">AI Clip Engine v2.0</span>
                    ...
                    <div class="grid grid-cols-2 gap-2 bg-slate-950 p-1.5 rounded-xl border border-slate-900">
                        <button id="tab-file" onclick="switchMode('file')" class="text-xs font-bold py-2 rounded-lg bg-slate-900 text-white shadow-lg border border-purple-500/10 transition-all">📁 Archivo</button>
                        <button id="tab-link" onclick="switchMode('link')" class="text-xs font-bold py-2 rounded-lg text-slate-500 transition-all">🔗 Enlace</button>
                    </div>
                ...
                <div id="step-processing" class="hidden py-12 text-center space-y-4">
                    ...
                    <p id="txt-processing" class="text-xs text-indigo-300 font-bold tracking-wide">Descargando video y procesando frecuencias de voz en tiempo real...</p>
                </div>
                ...
                <div id="step-results" class="hidden space-y-5">
                    ...
                    <button onclick="location.reload()" class="w-full text-center text-xs text-slate-500 hover:text-indigo-400 font-medium transition-colors block pt-2">Procesar otro archivo</button>
                </div>
            </div>
        </main>
        ...
        <script>
            ...
            const diccionario = {
                es: {
                    ...
                    processing: "Descargando video y procesando frecuencias de voz en tiempo real...",
                ...
                en: {
                    ...
                    processing: "Downloading video and modulating voice hooks in real-time...",
            ...
            function updateFileName() {
                ...
                if(fileInput.files.length > 0) label.innerText = fileInput.files[0].name;
            }
            ...
            document.getElementById('cleanerForm').addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    alert("Error en procesamiento. Verifica los logs de Render o que el formato sea válido.");
                    location.reload();
                }
            });
        </script>
...
async def process_clip(
    ...
    try:
        if file:
            ...
            raise HTTPException(status_code=400, detail="No se cargó ningún archivo o enlace.")

        # Comando FFmpeg seguro
        cmd = f"ffmpeg -i '{input_file}' -af 'highpass=f=100, lowpass=f=4500' -c:v copy '{output_video_path}' -y"
        process = await asyncio.create_subprocess_shell(cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        stdout, stderr = await process.communicate()

        if process.returncode != 0:
            print(f"FFmpeg Error: {stderr.decode()}")
            raise HTTPException(status_code=500, detail="Error de procesamiento de video (FFmpeg).")
        ...
        print("--- DETALLE DEL ERROR EN RENDER ---")
        traceback.print_exc()
...
async def download_file(session_id: str):
    ...
    raise HTTPException(status_code=404, detail="El enlace ha expirado o el archivo fue borrado.")
